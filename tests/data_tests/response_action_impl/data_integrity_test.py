...
def test_check_all_entities():
    ...
    for e_file in e_list:
        response_action_implementation = ResponseActionImpl.from_file(
            f"{base_data_dir}/{lang}/{e_dir}/{e_file}/{e_file}.yml"
        )
        assert response_action_implementation is not None
        for field in mandatory_fields:
            assert field in response_action_implementation.view
